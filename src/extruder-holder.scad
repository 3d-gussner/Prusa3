  ...
  difference() {
    union() {
      ...
      for (i=[0,1]) mirror([i,0,0]) hull() {
        ...
        translate([(47/2)-(1/2),(47/2)-(20/2),30/2]) cube([1,20,15], center=true);
      }
    }
    translate([0,0,(40/2)+2]) cube([47-4,47-4,40], center=true);
    union() {
      intersection() {
        translate([22,0,10]) cube([10,14,5.5], center=true);
        translate([22,0,4]) rotate([0,90,0]) cylinder(d=16.4, h=6, center=true, $fn=30);
      }
      translate([22,0,5.5]) cube([10,14,5.5], center=true);
    }
    translate([0,0,1]) cylinder(d=30, h=3, center=true);
    # for(i=[-1,1]) for(j=[-1,1]) translate([i*((52/2)+1.5), (47/2)-(2/2), (30/2)+8*j]) rotate([90,0,0]) cylinder(d=3.2, h=4, center=true, $fn=10);
    # for (a=[0:90:359]) rotate([0,0,a]) translate([15.5,15.5,0]) cylinder(d=3.2, h=5, center=true, $fn=10);
